...
/**
 * @description 获取本地存储 - 键, 值, 类型
 */
export const getStorage = (key, type = 'localStorage') => {
  const storage = {localStorage, sessionStorage}
  return storage[type][key] || false
}

/**
 * @description 设置本地存储 - 键, 值, 类型
 */
export const setStorage = (key, value, type = 'localStorage') => {
  const storage = {localStorage, sessionStorage}
  if (value === '') storage[type].removeItem(key)
  else storage[type][key] = JSON.stringify(value)
}
